...
const drawCanvas = (text: string, color: string = '#00FF00'): void => {
...
const snoozeVibration = (): void => {
  ...
  let snoozeLeft = 90;
  drawCanvas(`Pause ${snoozeLeft}s`, '#FFA500');
...
const startHold = (): void => {
  if (!isVibrating.value && !isCounting.value) return;
  // 3秒達成 = 每100ms增加約3.33%
  holdId = setInterval(() => {
...
<template>
  <div class="gym-container" :class="{ 'is-vibrating': isVibrating }">
    <div class="timer-card">
      ...
      <div v-if="!isCounting && !isVibrating" class="controls">
